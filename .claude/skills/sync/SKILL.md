...
の一覧ブロックと `dashboard.html` を更新する一連の作業を、
...
返ってくる JSON の主な区分を頭に入れます。

- `new`: inbox にあるが state に未登録
- `modified`: ソースハッシュが変わった既存エントリ
- `output_missing`: state では ok なのに source/ に MD がない
- `failed_retained`: 過去に失敗して再挑戦すべきもの
- `orphan_sources`: state にあるが inbox から消えた
- `orphan_outputs`: state に載っていない `source/` 側の MD
- `tampered_outputs`: state 記録後に改変された MD

何も無いなら、以降の変換作業はスキップして 5. に進みます。

### 2. 変換する

`new` / `modified` / `output_missing` / (config.retry.auto が true の
...
`orphan_sources` に該当する state エントリと、それに紐づく `source/`
...
`dashboard.html` を生成します。状態・inbox 数・取り込み済み
...
- [ ] scan の主な区分 (`new` / `modified` / `output_missing` / `failed_retained` / `orphan_*` / `tampered_outputs`) に目を通した
- [ ] new / modified / output_missing を Markdown 化した
