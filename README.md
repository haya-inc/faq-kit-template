...
2. `new` / `modified` / `output_missing` / `failed_retained` を
   Markdown に変換
...
6. `python tools/knowledgekit.py update-readme` でこの README の source-index
   ブロックを最新化
7. `python tools/knowledgekit.py dashboard` で `dashboard.html` を
   生成 (サーバー不要、`file://` で開ける)
...
`python tools/knowledgekit.py dashboard` を実行すると、`dashboard.html`
